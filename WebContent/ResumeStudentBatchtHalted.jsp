...
{   
   ...
   {  
	   ...
	   document.getElementById("error").innerHTML = "Please Select Batch Status as Ongoing to resume student from Halt";
	   return false;  
   }  
}
...
{   
var trainerid = document.RegisterStudent.TrainerId;
...
<%String message = (String)request.getAttribute("ErrorMessage"); 
	...
	<% } %>
<form name="RegisterStudent" action="ResumeStudentBatchHaltedAction" method="post" onsubmit="return formValidation()">
...
<% ArrayList<Batch> getBatches = student.getBatchlist(); %>
<td><label for="BatchId">        <select name="BatchId" id="bid" onkeyup="batchid_validation()">
        <% for (int i = 0; i < getBatches.size(); i++)
       	   { 
        		Batch batch = (Batch) getBatches.get(i);%>
            	  <option value = "<%= batch.getBatchId()%>"><% out.println(batch.getBatchId());%></option>
       	<% }%> 
	  </select></label>
</td>
...
<td> <label for="status"><select name="BatchStatus" id="status" onkeyup="validbatchstatus()">
...
<td><label for="TrainerId">     Trainer Id:</label></td>
        <%    ArrayList<Trainer> getTrainers = student.getTrainers(); %>
  <td><label for="Id">       <select name="TrainerId" id="Tid" onkeyup="validtrainerid()">
        <% for (int i = 0; i < getTrainers.size(); i++)
          ...
          <br>
<label for="h1">Previous Allocated Batch Details:-</label><br>
	
	<table border="1">
       <tr>
          <th><label for="tableB">Batch ID </label></th>
          <th><label for="tableB"> Batch Start Date	</label></th>
          <th><label for="tableB"> Batch End Date </label></th>
          <th><label for="tableB"> Batch Status </label></th>
       </tr>
       <tr>
	       <td><label for="tableB"><%= student.batch.getBatchId() %></label></td>
	       ...
	       <td><label for="tableB"><%= Startmdy %></label></td>
	       ...
	       <td><label for="tableB"><%= Endmdy %></label></td>
	       
	       <td><label for="tableB"><%= student.batch.getBatchStatus() %></label></td>
	       </tr>	
       </table>
       <br>
<label for="h2">Currently Available Batch Details:-</label><br>
     
    <%    ArrayList<Batch> Batchlist = student.getBatchlist(); %>
       <table border="1">
       <tr>
       ...
       {%>
	       <tr>
	       <td><label for="tableB"><%= serialno %> </label></td>
	       <td><label for="tableB"><%= batch.getBatchId() %> </label></td>
	       
	       <% Date BStartDate = batch.getBatchStartDate();
             SimpleDateFormat BStartmdyFormat = new SimpleDateFormat("yyyy-MM-dd");
             String BStartmdy = BStartmdyFormat.format(BStartDate);
           %>
	       <td><label for="tableB"><%= BStartmdy %> </label></td>
	       
	       <% Date BEndDate = batch.getBatchEndDate();
              SimpleDateFormat BEndmdyFormat = new SimpleDateFormat("yyyy-MM-dd");
   			  String BEndmdy = BEndmdyFormat.format(BEndDate);
 			%>
	       <td><label for="tableB"><%= BEndmdy %></label></td>
	       
	       <td><label for="tableB"><%= batch.getBatchStatus() %></label></td>
	       </tr>	
	       <% ++serialno; %>
       <% }%>
       </table>
      <br>
<label for="h2">   Previous Allocated Trainer Details:- </label><br>
	...
	<table border="1">
       ...
       <tr>
	       <td><label for="tableB"><%= student.trainer.getTrainerId() %></label></td>
	       <td><label for="tableB"><%= student.trainer.getTrainerFirstName() %></label></td>
	       <td><label for="tableB"><%= student.trainer.getTrainerLastName() %></label></td>
	       <td><label for="tableB"><%= student.trainer.getAvailability() %></label></td> 
	       <td><label for="tableB">
	       <%     String[] trainerspec = student.trainer.getTrainerSpecialization();
	              for(int i=0; i < trainerspec.length; i++) 
	       	      {
	            	  out.println(trainerspec[i]+" ");
	       	}%></label>
	       </td>
	       <td><label for="tableB"><%= student.trainer.getTrainerStatus() %></label></td>
	       <td><label for="tableB"><%= student.trainer.getTrainerWorkStatus() %></label></td>
	       </tr>	
       </table>
      <br>
<label for="h2">     Currently Available Trainer Details:- </label><br>
    
       <% ArrayList<Trainer> Trainers = student.getTrainers(); %>
       <table border="1">
       <tr>
          <th><label for="tableB"> S.No. </label></th>
          <th><label for="tableB"> Trainer ID </label></th>
